...
} from "../../../../Redux/whoWeAre/office/officeApi";
...
export default function OfficesSection() {
  ...
  const { data: privacy, isLoading } = useGetOfficeQuery();

  const [addPrivacy, { isLoading: addIsLoading }] = useAddOfficeMutation();
  ...
  const handlePrivacyPolicy = async (e) => {
    ...
    try {
      if (id) {
        ...
        if (res?.data?.success) {
          Swal.fire("Success", "Ofiice Info updated successfully", "success");
        } else {
        ...
        if (res?.data?.success) {
          Swal.fire("Success", "Ofiice Info created successfully", "success");
        } else {
          Swal.fire("Error", "Something went wrong", "error");
        }
      }
  ...
  return (
    <div className="make_privacy_policy">
      <h2 className="mb-3 text-center text-xl font-medium text-primary sm:text-2xl">
        {id ? "Edit Office Info" : "Create Office Info"}
      </h2>
